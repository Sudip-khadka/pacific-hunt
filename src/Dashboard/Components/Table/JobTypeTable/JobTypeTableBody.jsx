...
import UpdateJobType from '../../PopUps/UpdateJobType';
...
function JobTypeTableBody({ data, selectedRows, onCheckboxChange, onSelectAll, onSwitchToggle,refetchData ,currentPage, rowsPerPage  }) {
  ...
  return (
      ...
      <tbody>
        {data.map((item, index) => (
          <StyledTr key={item.id} $isDisabled={!switchStates[item.id]}>
            ...
            <StyledTd $isDisabled={!switchStates[item.id]}>{item.jobType}</StyledTd>
    ...
    {selectedJobType && (
        <UpdateJobType
        title="Update JobType"
          open={openUpdateDialog}
          onClose={handleUpdateDialogClose}
          onSubmit={handleJobTypeUpdate}
...
export default JobTypeTableBody;
