...
function SkillsTableBody({ data, selectedRows, onCheckboxChange, onSelectAll, onSwitchToggle,refetchData ,currentPage, rowsPerPage  }) {
      ...
      <tbody>
        {data.map((item, index) => (
          <StyledTr key={item.id} $isDisabled={!switchStates[item.id]}>
            ...
            <StyledTd $isDisabled={!switchStates[item.id]}>{item.skillsName}</StyledTd>
