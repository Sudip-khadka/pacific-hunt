...
const SocialMediaPopup = ({ open, onClose, onSubmit, title }) => {
  ...
  const mutation = useMutation({
    mutationFn: (newSocialMedia) => apiClient.post('socialMedia', newSocialMedia),
    onSuccess: () => {
      setShowAlert(true);
      setTimeout(() => {
        setShowAlert(false);
  ...
  <img src={enteredIcon} alt="Social Media Icon" style={{ width: '70px', height: '70px' }} />
      ...
      <div className="submitbtn p-6 text-[#fff] font-medium text-base flex flex-col gap-3">
        {showAlert && <Alert message="Social Media added successfully!" />}
        <Buttons width="512px" onClick={handleSubmit} type="create" title="Create" />
      </div>
    </StyledDialog>
  );
};
