...
function EmployeerNavigation() {
  const [notification, setNotification] = useState(true);
  const [employees, setEmployees] = useState([]);

  useEffect(() => {
    const storedData = localStorage.getItem("employeerDetail");
    if (storedData) {
  ...
  return (
    <Navbar>
          <Profile>
            <img src={employees.companyLogo} alt="Profile Image" />
             <p className="username">{employees?.companyName || "No Company"}</p>
          </Profile>
      {notification ? (
...
export default EmployeerNavigation;
